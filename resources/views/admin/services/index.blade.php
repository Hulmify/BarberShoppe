...
<div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
    ...
    <form method="GET" action="{{ route('admin.services.index') }}" class="w-full md:w-1/2">
        ...
        <div class="relative">
            ...
            <input type="search" id="search" name="search" value="{{ request('search') }}" class="block w-full p-4 ps-10 text-sm text-slate-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-primary-500 focus:border-primary-500" placeholder="Search services..." />
    ...
    <!-- Add Button -->
    <a href="{{ route('admin.services.create') }}" class="w-full md:w-auto text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-3 focus:outline-none flex items-center justify-center gap-2 transition-colors shadow-sm">
...
<div class="relative overflow-x-auto shadow-sm sm:rounded-xl border border-gray-200">
    @if($services->isEmpty())
        <div class="flex flex-col items-center justify-center p-12 text-center bg-white">
            ...
            <a href="{{ route('admin.services.create') }}" class="text-primary-600 hover:text-primary-700 font-bold hover:underline">Add a new service &rarr;</a>
        </div>
    @else
                ...
                <tr class="bg-white hover:bg-gray-50 transition-colors">
                    ...
                    <td class="px-6 py-4 text-right">
                        <div class="flex justify-end gap-2">
                             <a href="{{ route('admin.services.edit', $service->id) }}" class="p-2 text-slate-500 hover:text-primary-600 hover:bg-primary-50 rounded-lg transition-colors border border-transparent hover:border-primary-200" title="Edit Service">
